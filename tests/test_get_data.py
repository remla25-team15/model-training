...
def test_download_nltk_resources_mocks_nltk():
    ...
    with mock.patch("src.get_data.nltk.data.find", side_effect=LookupError), \
         mock.patch("src.get_data.nltk.download") as mock_download:
        result = download_nltk_resources()
        mock_download.assert_called_once_with("wordnet")
        assert result is True

    with mock.patch("src.get_data.nltk.data.find", return_value=True), \
         mock.patch("src.get_data.nltk.download") as mock_download:
        result = download_nltk_resources()
...
def test_run_get_data_mocks_everything():
    with mock.patch("src.get_data.download_from_drive") as mock_drive, \
         mock.patch("src.get_data.download_nltk_resources", return_value=True) as mock_nltk:
        file_ids = ["id1", "id2"]
        output_names = ["file1.txt", "file2.txt"]
        out_dir = "some_dir"
        result = run_get_data(output_dir=out_dir, file_ids=file_ids, output_names=output_names)
