...
def main():
    parser = argparse.ArgumentParser(
        description="Train a sentiment analysis model for restaurant reviews."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="../datasets/a1_RestaurantReviews_HistoricDump.tsv",
        help="Path to the dataset (TSV file).",
    )
    parser.add_argument(
        "--train_all",
        action="store_true",
        help="Train on the full dataset without evaluation.",
    )
    parser.add_argument(
        "--output",
        type=str,
